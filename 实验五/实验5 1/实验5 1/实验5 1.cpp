...
#include "stdafx.h"
	...
	int main()
	{
		...
		Sort(&fScore[0][0], iNumber, fAver, fSum, iTotal);
		int k, m;
		for (int i = 0; i <iTotal; i++)
		{
			printf("第%d名学号为%d\t总分为:%f\t数学为:%f\t英语为:%f\t物理为:%f\t平均分为:%f\n", i+1, iNumber[i],fSum[i], fScore[i][0], fScore[i][1], fScore[i][2], fAver[i]);
			irank[iNumber[i]] =i+1;
		}
		printf("输入查找学号：\n");
		scanf_s("%d",&k);
